...
void save_seat_data(Seat seats[], int num_seats);
void busroutemanagement();
void initializeSeats(Seat seats[], int num_seats);
void displaySeatStatus(Seat seats[], int num_seats);
void updateSeatStatus(Seat seats[], int num_seats);
void updatePrices();
void browseSeats();
...
int travelerLogin(User *traveler) {
    ...
    if (file != NULL) {
        ...
        while (fscanf(file, "%s %s %s %s\n", tempUsername, tempPassword, tempSecurityQuestion, tempSecurityAnswer) == 4) {
...
void travelerRegister(User *traveler) {
    ...
    scanf(" %[^\n]", security_question); // Read the entire line including spaces
    printf("\n                                        Enter security answer: ");
    scanf(" %[^\n]", security_answer); // Read the entire line including spaces
...
void resetPassword(User *traveler) {
    ...
    if (strcmp(security_answer, traveler->security_answer) == 0) {
        ...
        FILE *file = fopen("travelers.txt", "r+");
        if (file != NULL) {
            char tempUsername[MAX_USERNAME_LENGTH], tempPassword[MAX_PASSWORD_LENGTH], tempSecurityQuestion[100], tempSecurityAnswer[100];
            FILE *tempFile = fopen("temp.txt", "w");
            if (tempFile != NULL) {
                while (fscanf(file, "%s %s %s %s\n", tempUsername, tempPassword, tempSecurityQuestion, tempSecurityAnswer) != EOF) {
                    if (strcmp(tempUsername, traveler->username) == 0) {
                        fprintf(tempFile, "%s %s %s %s\n", tempUsername, new_password, tempSecurityQuestion, tempSecurityAnswer);
                    } else {
                        fprintf(tempFile, "%s %s %s %s\n", tempUsername, tempPassword, tempSecurityQuestion, tempSecurityAnswer);
                    }
                }
                fclose(tempFile);
            }
            fclose(file);
            remove("travelers.txt");
            rename("temp.txt", "travelers.txt");
...
void seatmanagement() {
    ...
    do {
        ...
        switch (choice) {
            case 1:
                displaySeatStatus(seats, num_seats);
                break;
            case 2:
                updateSeatStatus(seats, num_seats);
                break;
            case 3:
                printf("\nExiting...\n");
                break;
...
void displaySeatStatus(Seat seats[], int num_seats) {
    ...
    for (int i = 0; i < num_seats; i++) {
        fscanf(file, "%d %d", &seats[i].seat_number, &seats[i].is_available);
        printf("Seat %d - %s\n", seats[i].seat_number, seats[i].is_available ? "Available" : "Booked");
    }
...
void updateSeatStatus(Seat seats[], int num_seats) {
    ...
    for (int i = 0; i < num_seats; i++) {
        fprintf(file, "%d %d\n", seats[i].seat_number, seats[i].is_available);
    }

    fclose(file);
}

// void display_available_seats(Seat seats[], int num_seats) {
    // Display available seats
    // printf("Available Seats:\n");
    // for (int i = 0; i < num_seats; i++) {
        // if (seats[i].is_available) {
            // printf("Seat %d - Price: $%.2f\n", seats[i].seat_number, seats[i].price);
        // }
    // }
// }
void make_reservation(Seat seats[], int num_seats, User user) {
    ...
    printf("\nEnter seat numbers you want to reserve (0 to stop): ");
    int seat_number;
    scanf("%d", &seat_number);
    
    while (seat_number != 0) {
        if (seat_number >= 1 && seat_number <= num_seats && seats[seat_number - 1].is_available) {
            selected_seats[num_selected_seats++] = seat_number;
            seats[seat_number - 1].is_available = 0; // Mark seat as reserved
        } else {
            printf("Invalid seat number or seat not available.\n");
        }
        
        printf("Enter next seat number (0 to stop): ");
        scanf("%d", &seat_number);
    }
    
    // Calculate total cost
    calculate_total_cost(seats, num_seats, selected_seats);
}
...
void save_seat_data(Seat seats[], int num_seats) {
    ...
    for (int i = 0; i < num_seats; i++) {
        fprintf(fp, "%d %.2f %d\n", seats[i].seat_number,seats[i].price, seats[i].is_available);
    }
...
void displayBusRoutes() {
    // Read bus routes from file and display them
FILE *file = fopen("bus_routes.txt", "r");
    ...
    while (fscanf(file, "%s %s %s %s %s %s %f", bus_name, route_name, origin, destination, arrival_time, departure_time, &ticket_price) == 7) {
        printf("Bus Name: %s\n", bus_name);
        printf("Route Name: %s\n", route_name);
        printf("Origin: %s\n", origin);
...
void AddBusRoute() {
    ...
    fprintf(file, "%s %s %s %s %s %s %.2f\n", newRoute.bus_name, newRoute.route_name, newRoute.origin, newRoute.destination,newRoute.arrival_time, newRoute.departure_time,newRoute.ticket_price);
...
void UpdateBusRoute() {
    // Display current bus routes
    printf("\nCurrent Bus Routes:\n");
    displayBusRoutes();

    char route_name[MAX_ROUTE_NAME_LENGTH];
    printf("Enter the name of the route to update: ");
    scanf("%s", route_name);
    ...
    while (fscanf(file, "%s %s %s %s %s %s %f", route, origin, destination, arrival_time, departure_time, bus_name, &ticket_price) == 7) {
        if (strcmp(route, route_name) == 0) {
            found = 1;
            // Prompt for and update route details
        ...
        fprintf(tempFile, "%s %s %s %s %s %s %.2f\n", route, origin, destination, arrival_time, departure_time, bus_name, ticket_price);
    }
...
void DeleteBusRoute() {
    // Prompt for and collect the name of the route to be deleted
    char routeName[MAX_ROUTE_NAME_LENGTH];
    printf("\nEnter the name of the route you want to delete: ");
    scanf("%s", routeName);
    ...
    // Open a temporary file in write mode to store routes excluding the one to be deleted
    FILE *tempFile = fopen("temp.txt", "w");
    if (tempFile == NULL) {
        printf("Error opening temporary file for writing.\n");
        fclose(file);
    ...
    // Search for the specified route in the file and exclude it while copying other routes to the temporary file
    int found = 0;
    while (fscanf(file, "%s %s %s %s %s %s", currentRoute.bus_name, currentRoute.route_name, currentRoute.origin, currentRoute.destination,currentRoute.arrival_time, currentRoute.departure_time) != EOF) {
        // Check if the current line corresponds to the route to be deleted
        if (strcmp(currentRoute.route_name, routeName) == 0) {
            found = 1;
            continue; // Skip writing this route to the temporary file
        }
        // Write the current line (excluding the route to be deleted) to the temporary file
        fprintf(tempFile, "%s %s %s %s %s %s\n", currentRoute.bus_name, currentRoute.route_name, currentRoute.origin, currentRoute.destination,currentRoute.arrival_time, currentRoute.departure_time);
    }
